...
export default {
  ...
  methods: {
    ...
    changeEnable(row) {
      this.request.post("/warn-info/update", row).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/warn-info/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/warn-info/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    save() {
      this.request.post("/warn-info", this.form).then(res => {
        if (res.code === '200') {
